...
code = code.replace(/export const loadFromStorage = <T>\(key: string, value: T\): T => \{[\s\S]*?\};\n?/, newLoad);
// Wait, the regex might be slightly wrong. Let's just do a string replacement.
fs.writeFileSync('patch.js', `
const fs = require('fs');
let code = fs.readFileSync('src/mockData.ts', 'utf-8');
