        ...
        <div className="bg-white dark:bg-[#111625] rounded-xl p-6 border border-slate-200 dark:border-slate-800 shadow-sm max-w-3xl mx-auto">
          ...
          <form onSubmit={handleCreateMeet} className="space-y-4">
            ...
            <div className="space-y-1">
              ...
              <div className="flex flex-wrap gap-2 pt-1">
                {schoolClasses.map(cls => (
                  <label key={cls} className={`flex items-center gap-1.5 px-3 py-2 rounded-lg border cursor-pointer text-xs transition-colors ${selectedClasses.includes(cls) ? 'bg-emerald-50 dark:bg-emerald-900/30 border-emerald-200 dark:border-emerald-800 text-emerald-700 dark:text-emerald-400' : 'bg-slate-50 dark:bg-slate-800/50 border-slate-200 dark:border-slate-700 text-slate-600 dark:text-slate-400'}`}>
                    <input 
                      type="checkbox"
                      className="hidden"
            ...
            <div className="pt-4 flex justify-end">
              <button 
                type="submit" 
                className="px-6 py-2.5 bg-emerald-600 hover:bg-emerald-700 text-white text-xs font-bold rounded-lg shadow-sm flex items-center gap-2 transition-all cursor-pointer"
              >
                <Video size={16} />
                Bagikan Jadwal Meet
              </button>
        ...
        <div className="space-y-4">
          {filteredMeets.length === 0 ? (
            <div className="bg-white dark:bg-[#111625] rounded-xl p-10 text-center border border-dashed border-slate-200 dark:border-slate-800 text-slate-400">
               <Video size={32} className="mx-auto mb-3 text-emerald-400 opacity-50" />
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
              {filteredMeets.map(meet => {
                ...
                return (
                  <div key={meet.id} className={`p-5 rounded-xl border relative transition-all shadow-sm ${isUpcoming ? 'bg-white dark:bg-[#111625] border-emerald-100 dark:border-emerald-800/50' : 'bg-slate-50 dark:bg-slate-800/40 border-slate-200 dark:border-slate-800 opacity-80'}`}>
                    {activeRole === 'guru' && (
                      <button 
                        onClick={() => {
                    ...
                    <div className="flex items-center gap-2 mb-3">
                      <div className={`p-2 rounded-lg ${isUpcoming ? 'bg-emerald-50 text-emerald-600 dark:bg-emerald-900/30 dark:text-emerald-400' : 'bg-slate-100 text-slate-500 dark:bg-slate-800'}`}>
                        <Video size={16} />
                      </div>
                      <div>
                    ...
                    <div className="mt-4 pt-4 border-t border-slate-100 dark:border-slate-800">
                      <a 
                        ...
                        className={`w-full py-2.5 flex items-center justify-center gap-2 text-xs font-bold rounded-lg transition-colors cursor-pointer ${isUpcoming ? 'bg-emerald-600 hover:bg-emerald-700 text-white shadow-sm shadow-emerald-600/20' : 'bg-slate-100 hover:bg-slate-200 dark:bg-slate-800 dark:hover:bg-slate-700 text-slate-600 dark:text-slate-300'}`}
                      >
                        <Video size={14} />
                        Join Google Meet
                      </a>
