...
export default function BulkPrintCards({ students, schoolIdentity, onClose }: BulkPrintCardsProps) {
  ...
  const classes = ['Semua', ...Array.from(new Set(students.map(s => s.className)))];
  ...
  // Select all / Deselect all for filtered students
  ...
  // Pre-select all students of the current filtered list by default on open
  useEffect(() => {
  ...
  useEffect(() => {
    const generateQRs = async () => {
      ...
      for (const studentId of selectedStudentIds) {
        if (!newQrs[studentId]) {
          try {
            const url = await QRCode.toDataURL(studentId || "N/A", { margin: 1, width: 120 });
            newQrs[studentId] = url;
            updated = true;
          } catch (err) {
  ...
  const getStudentTheme = (studentClass: string): CardTheme => {
  ...
  const selectedStudentsToPrint = students.filter(s => selectedStudentIds.includes(s.id));

  // Paginate selected students into pages of 4 cards
  const pagesOfStudents: Student[][] = [];
  for (let i = 0; i < selectedStudentsToPrint.length; i += 4) {
    pagesOfStudents.push(selectedStudentsToPrint.slice(i, i + 4));
  }
  ...
  const handleBulkDownloadPng = async () => {
    if (selectedStudentsToPrint.length === 0) return;
    setDownloadingPngs(true);
    setDownloadSuccess(false);
    setDownloadTotal(selectedStudentsToPrint.length);
    ...
    try {
      ...
      for (let i = 0; i < selectedStudentsToPrint.length; i++) {
        const student = selectedStudentsToPrint[i];
        
        // Find the element by its ID
        const elementId = `print-card-actual-${student.id}`;
        const cardElement = document.getElementById(elementId);
        
        if (cardElement) {
          // Temporarily remove scaling if any, though html-to-image handles it mostly.
          ...
          const fileName = `${student.className.replace(/\s+/g, '_')}_${student.name.toLowerCase().replace(/\s+/g, '_')}_${student.id}.png`;
      ...
      link.download = `kartu_id_png_massal_${Date.now()}.zip`;
    ...
    <div className="fixed inset-0 z-[9999] bg-slate-950/90 flex flex-col h-screen print:static print:h-auto print:min-h-auto print:block print:bg-white print:p-0 print:m-0 print:overflow-visible print-active-overlay">
      ...
      <div className="p-4 md:p-5 bg-white dark:bg-[#1a1b2e] border-b border-slate-200/60 dark:border-slate-800/60 flex flex-col md:flex-row justify-between items-start md:items-center gap-4 shrink-0 shadow-sm print:hidden">
        ...
        <div className="flex items-center gap-2 w-full md:w-auto">
          ...
          <button
            ...
            disabled={selectedStudentsToPrint.length === 0 || downloadingPngs}
      ...
      <div className="flex flex-col lg:flex-row flex-1 overflow-hidden print:hidden">
        ...
        <div className="w-full lg:w-96 bg-slate-50 dark:bg-[#11121e] border-r border-slate-200/50 dark:border-slate-800/50 flex flex-col h-1/2 lg:h-full shrink-0 overflow-hidden">
          ...
          {/* Student Selectors List */}
          <div className="flex-1 overflow-y-auto p-4 space-y-2">
            <div className="flex justify-between items-center pb-2 border-b border-slate-100 dark:border-slate-800/80">
              <span className="text-[11px] font-bold text-slate-400">
                Siswa ({filteredStudents.length} Terfilter)
              </span>
              <button
              ...
              >
                {filteredStudents.every(s => selectedStudentIds.includes(s.id)) ? 'Hapus Semua' : 'Pilih Semua'}
              </button>
            </div>
            ...
            {filteredStudents.length === 0 ? (
              ...
              <div className="space-y-1">
                {filteredStudents.map(student => {
                  ...
                  const stTheme = getStudentTheme(student.className);
                  return (
                    <button
                      key={`bulk-select-${student.id}`}
                    ...
                    >
                      <div className="flex items-center gap-2.5 min-w-0">
                        ...
                        <div className="min-w-0">
                          <p className="text-xs font-bold text-slate-800 dark:text-slate-100 truncate">{student.name}</p>
                          <p className="text-[10px] text-slate-400 font-mono">NIS: {student.id}</p>
                        </div>
                      </div>
          ...
          <div className="p-4 bg-white dark:bg-[#161726] border-t border-slate-200/60 dark:border-slate-800/60 text-xs space-y-3 shrink-0 text-slate-500 dark:text-slate-400">
            <div className="space-y-1">
              <div className="flex justify-between font-bold">
                ...
                <span className="text-emerald-600 dark:text-emerald-400">{selectedStudentIds.length} Siswa</span>
              </div>
              <div className="flex justify-between text-[11px]">
                <span>Jumlah Lembar A4:</span>
                <span className="font-semibold">{pagesOfStudents.length} Halaman</span>
              </div>
            </div>
        ...
        <div className="flex-1 bg-slate-200 dark:bg-slate-950 overflow-y-auto p-4 md:p-8 flex flex-col items-center gap-8 shadow-inner">
          <div className="max-w-[750px] w-full bg-white dark:bg-[#1a1b2e] border border-slate-300/60 dark:border-slate-800/60 p-5 rounded-2xl shadow-sm text-left space-y-4 no-print">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4 text-xs">
              ...
              <div className="space-y-2 bg-slate-50 dark:bg-[#131422] p-3 rounded-xl border border-slate-100 dark:border-slate-800/40">
                ...
                <p className="text-[11px] text-slate-600 dark:text-slate-400 leading-relaxed">
                  Gunakan kertas berkualitas tinggi untuk menjamin daya tahan kartu yang dipegang siswa sehari-hari:
                </p>
                <ul className="space-y-1 list-disc list-inside text-[10.5px] text-slate-500 dark:text-slate-400 pl-1">
          ...
          {selectedStudentsToPrint.length === 0 ? (
            <div className="flex-1 flex flex-col items-center justify-center p-8 text-slate-400">
              ...
              <p className="text-xs text-center max-w-sm mt-1">Centang daftar siswa di panel sebelah kiri untuk memunculkan preview kartu di halaman A4.</p>
            </div>
          ) : (
            pagesOfStudents.map((page, pIdx) => (
              ...
              const stTheme = getStudentTheme(student.className);
              return (
                <div id={`print-card-actual-${student.id}`} key={`print-card-actual-${student.id}`} style={{ transform: 'scale(1)', transformOrigin: 'top left', width: '310px', height: '512px', overflow: 'hidden' }}>
                  <StudentIdCard 
                    student={student} 
                    schoolIdentity={{ name: schoolName, city: schoolCity, logo: schoolLogo }} 
                    isPrintMode={true} 
                    forcedThemeId={stTheme.id}
                  />
                </div>
              );
            })}
    ...
    {pagesOfStudents.map((page, pIdx) => (
      <div 
        ...
        style={{
          ...
          padding: `${12 + offsetY}mm ${10 + offsetX}mm`,
          boxSizing: 'border-box',
          pageBreakAfter: pIdx < pagesOfStudents.length - 1 ? 'always' : 'auto',
          breakAfter: pIdx < pagesOfStudents.length - 1 ? 'page' : 'auto',
              ...
              <div className="p-6 flex flex-col items-center gap-6 bg-slate-100 dark:bg-slate-950 overflow-y-auto max-h-[60vh]">
                ...
                <div className="w-[210mm] min-h-[297mm] bg-white shadow-xl relative scale-50 md:scale-75 lg:scale-100 origin-top border border-slate-200">
                  ...
                  <div className="p-10 text-center relative z-10 space-y-4">
                    <p className="font-bold text-slate-800 text-xl mt-20">Siap Mencetak {selectedStudentsToPrint.length} Kartu</p>
